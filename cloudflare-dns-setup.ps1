...
function Show-Help {
    ...
    Write-Host "  .\cloudflare-dns-setup.ps1 -Login        # Authenticate with Cloudflare" -ForegroundColor White
    Write-Host "  .\cloudflare-dns-setup.ps1 -Setup        # Configure DNS records" -ForegroundColor White
    Write-Host "  .\cloudflare-dns-setup.ps1 -Verify       # Check DNS records" -ForegroundColor White
    Write-Host "  .\cloudflare-dns-setup.ps1 -Status       # Show account status" -ForegroundColor White
...
function Set-DNSRecords {
    ...
    # First, let's try to delete any existing A and CNAME records for the domain
    Write-Host "`n🗑️  Cleaning up existing records..." -ForegroundColor Yellow
    
    # Note: Wrangler doesn't have direct DNS record management in v3+
    # We'll need to use curl with Cloudflare API directly
    Write-Host "⚠️  Note: Wrangler v3+ doesn't support direct DNS management." -ForegroundColor Yellow
    Write-Host "We'll use the Cloudflare API directly via curl..." -ForegroundColor Gray
    
    # Check if curl is available
    try {
        curl --version | Out-Null
        if ($LASTEXITCODE -ne 0) {
            Write-Host "❌ curl is not available. Please install curl or use the Cloudflare dashboard." -ForegroundColor Red
            return
        }
    }
    catch {
    ...
    Write-Host "Run: .\cloudflare-dns-setup.ps1 -Verify to check the results" -ForegroundColor Cyan
}
...
if ($Setup) {
    if (Test-Authentication) {
        ...
        Write-Host "❌ Please authenticate first: .\cloudflare-dns-setup.ps1 -Login" -ForegroundColor Red
    }
}
...
Write-Host "1. Login: .\cloudflare-dns-setup.ps1 -Login" -ForegroundColor White
Write-Host "2. Setup: .\cloudflare-dns-setup.ps1 -Setup" -ForegroundColor White  
Write-Host "3. Verify: .\cloudflare-dns-setup.ps1 -Verify" -ForegroundColor White
